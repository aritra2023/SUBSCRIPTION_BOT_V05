...
FAMPAY_SCANNER_IMAGE = "https://files.catbox.moe/p8uccv.jpeg"
...
# ── QR prefetch cache ──────────────────────────────────────────────────────────
# (user_id, amt) → {"qr_id": str, "buf": BytesIO, "ts": float} | None (in-progress)
_qr_prefetch_cache: dict = {}

async def _prefetch_qr(user_id: int, amt: int):
    """Background task: generate Razorpay QR + download image before user taps 'Pay with QR'."""
    import httpx
    key = (user_id, amt)
    if key in _qr_prefetch_cache:
        return  # already generating or done
    _qr_prefetch_cache[key] = None  # sentinel: in-progress
    try:
        ...
        logger.error(f"QR prefetch failed user={user_id} amt={amt}: {e}")
        _qr_prefetch_cache.pop(key, None)
...
async def cmd_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pending_broadcasts[update.effective_user.id] = {
        ...
        "reply_markup": src.reply_markup,  # preserve inline buttons from original message
    }
    keyboard = [[
    ...
    for i, usr in enumerate(all_users):
        try:
            await context.bot.copy_message(
                ...
                reply_markup=info.get("reply_markup"),  # keep original buttons
            )
            success += 1
        except Exception as e:
...
async def handle_custom_recharge_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton(u("FamPay"),          callback_data=f"wfampay_{amt}"),
        ],
        [InlineKeyboardButton(u("🎧 Support"), url=f"https://t.me/{SUPPORT_USERNAME}")],
    ...
    asyncio.create_task(_prefetch_qr(update.message.from_user.id, amt))
...
async def wallet_amount_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(query, context, msg, keyboard)
    # Start pre-generating the QR in background so it's ready instantly when user taps
    asyncio.create_task(_prefetch_qr(query.from_user.id, amt))
...
async def wallet_pay_qr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        import httpx
        key = (query.from_user.id, amt)

        # ── Try cache first (pre-generated by _prefetch_qr) ──────────────────
        # Poll up to 8 s in case prefetch is still in-progress
        for _ in range(27):  # 27 × 0.3 s = ~8 s max wait
            cached = _qr_prefetch_cache.get(key)
            if cached is not None:          # None means in-progress; dict means done
                break
            if key not in _qr_prefetch_cache:
                break                       # prefetch never started or failed
            await asyncio.sleep(0.3)

        cached = _qr_prefetch_cache.pop(key, None)
        if cached and cached.get("qr_id"):
            qr_id  = cached["qr_id"]
            buf    = cached["buf"]
            buf.seek(0)
            gen_ok = True
            logger.info(f"QR served from cache for user {query.from_user.id} amt {amt}")
        else:
...
# ── Wallet recharge verification (manual — admin credits wallet) ───────────────
async def wallet_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ref_and_type = rest_rsplit[0]
    type_split   = ref_and_type.split("_", 1)
    ptype        = type_split[0]
...
async def wallet_pay_fampay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data.startswith("wfampay_"):     await wallet_pay_fampay(update, context)
